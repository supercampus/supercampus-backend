...
The `supercampus-notification-worker` delivers outbound CRM email, SMS,
WhatsApp, and Firebase Cloud Messaging push jobs. Run one or more replicas;
per-delivery row leases and `SKIP LOCKED` make concurrent replicas safe.
...
- Push: `FCM_ENABLED=true`, `FIREBASE_PROJECT_ID`, and
  `GOOGLE_APPLICATION_CREDENTIALS=/run/secrets/firebase-admin.json`

The Firebase JSON file is a runtime secret. Mount it read-only in the worker
container and do not put the JSON contents in an environment variable, build
argument, container image, application log, or Git repository.
...
1. Deploy migrations through 74 using the normal migration runner/API startup.
...
4. Open the mobile app once, grant notification permission, and verify an FCM
   device token appears in `campus_ops.push_devices`.
5. Send one controlled message per channel to institution-owned test recipients.
6. Confirm the communication/push-delivery row becomes `sent` and stores the
   provider message ID.
7. Confirm a deliberately invalid test recipient retries and becomes `failed`
