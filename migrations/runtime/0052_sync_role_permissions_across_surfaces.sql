-- Access Control treats WEB and APP as synchronized delivery surfaces.
-- WEB is authoritative for existing roles. APP is used only as a fallback for
-- legacy roles that have never had a WEB grant.
CREATE TEMP TABLE access_surface_source ON COMMIT DROP AS
SELECT
    tenant_id,
    role_id,
    CASE
...
CREATE TEMP TABLE authoritative_role_permissions ON COMMIT DROP AS
SELECT permission.*
FROM authz.role_permissions permission
JOIN access_surface_source source
...
USING access_surface_source source
...
FROM authoritative_role_permissions permission
